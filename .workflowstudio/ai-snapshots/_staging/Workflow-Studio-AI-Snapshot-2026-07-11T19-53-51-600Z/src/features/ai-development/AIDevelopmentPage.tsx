...
export function AIWorkspacePage({ activePage, activeWorkspace }: AIDevelopmentPageProps) {
  ...
  const packageButtonLabel = useMemo(() => {
    ...
    if (!gitStatus?.isRepository) return "Git Status Required";
    if ((gitStatus.changedFiles.length ?? 0) === 0) return "No Workspace Changes Detected";
    if (packageReadiness.state === "blocked") return "Package Not Ready";
    return "Build Package From Workspace Changes";
  }, [developerRequest, gitStatus, packageReadiness.state, rootPath]);
    ...
    <>
      ...
      {activeTab === "package" && (
        <section className="module-panel ai-package-builder-panel ai-tab-panel">
          ...
          <div className="ai-package-builder-grid">
            <div><strong>Package preview</strong><ul><li>Target workspace: {analysis?.projectName ?? activeWorkspace?.name ?? "No workspace selected"}</li><li>Package ID: {packageId.trim() || "Auto-generated from project and timestamp"}</li><li>Build command: {analysis?.buildCommand ?? "npm run build"}</li><li>Output folder: _packages/{packageId.trim() || "generated-package-id"}</li></ul></div>
            <div><strong>Detected replacement files</strong>{gitStatus?.changedFiles.length ? <ul>{gitStatus.changedFiles.slice(0, 10).map((file) => <li key={file}>{file}</li>)}{gitStatus.changedFiles.length > 10 && <li>+ {gitStatus.changedFiles.length - 10} more</li>}</ul> : <p>No Git changes detected yet.</p>}</div>
          </div>
          {packageResult && <div className={`ai-package-result ${packageResult.safetyState ?? (packageResult.ok ? "safe" : "blocked")}`}>
            ...
            {packageResult.files?.length ? <div className="ai-package-file-list"><strong>Included files</strong><ul>{packageResult.files.map((file) => <li key={file}>{file}</li>)}</ul></div> : null}
      ...
      {activeTab === "snapshots" && (
        <section className="module-panel ai-tab-panel">
          <div className="ai-section-heading"><div><p className="eyebrow">Workspace History</p><h3>AI Snapshots</h3></div><div className="ai-snapshot-actions"><button type="button" className="ai-inline-action" onClick={() => refresh().then(() => showStatus("Snapshot history refreshed."))} disabled={!rootPath || isWorking}>Refresh History</button><button type="button" className="ai-inline-action" onClick={handleOpenSnapshotFolder} disabled={!rootPath}>Open Snapshot Folder</button></div></div>
