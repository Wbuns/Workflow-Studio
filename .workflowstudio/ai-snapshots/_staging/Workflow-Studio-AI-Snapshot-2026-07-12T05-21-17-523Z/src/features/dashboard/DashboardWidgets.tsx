  ...
  return (
    <div className="project-workspace-layout">
      <section className="workspace-status-grid" aria-label="Workspace status">
        ...
        <Stat label="Readiness" value={`${summary.readinessScore}%`} detail={summary.readinessStatus} />
      ...
      <section className="project-workspace-main">
        <article className="workspace-card workspace-recommendations-card">
          ...
          <div className="workspace-recommendation-list">
            {summary.recommendations.slice(0, 4).map((item) => (
      ...
      <section className="workspace-card">
        ...
        <div className="workspace-readiness-grid">
          {summary.readinessCategories.map((category) => <article key={category.id}><span>{category.label}</span><strong>{category.score === undefined ? category.status : `${category.score}%`}</strong><p>{category.detail}</p></article>)}
        </div>
      </section>
      ...
      <details className="workspace-card workspace-technical-details">
        <summary>Technical Details and Workspace Commands</summary>
        ...
        {(execution || executionError || output.length > 0) && <div className="workspace-command-console"><div><strong>{execution?.commandLabel ?? "Command Console"}</strong>{execution?.status === "running" && <button className="secondary-button" type="button" onClick={cancelCommand}>Stop</button>}</div>{executionError && <p>{executionError}</p>}<pre>{output.map((entry) => entry.text).join("")}</pre></div>}
      </details>
    </div>
  );
}
