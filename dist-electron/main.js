    ...
    child.on("close", (code, signal) => {
        ...
        if (buildRecordIndex >= 0) {
            ...
            automationRecords[buildRecordIndex] = {
                ...
                durationMs: Math.max(0, Date.parse(running.execution.finishedAt) - Date.parse(buildRecord.startedAt)),
...
function newestDownloadedWorkflowPackage() {
...
function extractPackageArchive(zipPath) {
...
function detectExtractedPackageRoot(extractedPath) {
    if (fs.existsSync(path.join(extractedPath, "manifest.json")))
...
function installDeveloperPackage(rootPathInput, zipPath) {
    ...
    const result = (ok, message, details) => {
    ...
    try {
        ...
        const prepared = manifest.files.map((file, index) => {
            if (!file.source || !file.target)
                throw new Error(`Manifest file entry ${index + 1} is missing source or target.`);
            const source = path.resolve(packageRoot, file.source);
            const target = path.resolve(rootPath, file.target);
            if (!isPathInside(packageRoot, source))
                throw new Error(`Manifest source path escapes the package root: ${file.source}`);
            if (!isPathInside(rootPath, target) || target === rootPath)
                throw new Error(`Manifest destination path is invalid: ${file.target}`);
            if (!fs.existsSync(source) || !fs.statSync(source).isFile())
                throw new Error(`Package source is missing or is not a file: ${file.source}`);
            if (fs.statSync(source).size <= 0)
                throw new Error(`Package source is empty: ${file.source}`);
        ...
        if (duplicate)
            throw new Error(`Package manifest contains a duplicate destination path: ${prepared[normalizedTargets.indexOf(duplicate)].targetLabel}`);
        ...
        for (const file of prepared) {
            ...
            try {
                ...
                if (!targetStat.isFile() || targetStat.size <= 0)
                    throw new Error("Destination file is missing or empty after copy.");
                if (!filesMatch(file.source, file.target))
                    throw new Error("Destination content does not match the package source.");
                operation.verified = true;
            }
            catch (error) {
        ...
        if (installedCount === 0)
            throw new Error("No files were installed. Every manifest entry was skipped or failed.");
        if (operations.some((operation) => !operation.verified))
            throw new Error("Post-install verification failed for one or more files.");
        ...
        if (rollbackEntries.length > 0)
            validationSummary.push(rollbackErrors.length ? "Rollback completed with errors." : "Partial installation rolled back.");
...
async function chooseAndInstallDeveloperPackage(rootPath) {
...
function latestSuccessfulBuild(rootPath) {
    return readDeveloperAutomationHistory().find((record) => record.action === "build" && record.rootPath === rootPath && record.status === "success");
}
function suggestedDeveloperCommitMessage() {
    const installRecord = readDeveloperAutomationHistory().find((record) => record.action === "install-package" && record.status === "success" && record.packageId);
    if (!installRecord?.packageId)
        return "Update active workspace";
...
function getDeveloperGitAutomationState(rootPathInput) {
    const rootPath = normalizeRoot(rootPathInput);
    const status = getGitStatus(rootPath);
...
function commitDeveloperChanges(rootPathInput, message) {
    const rootPath = normalizeRoot(rootPathInput);
...
function pushDeveloperBranch(rootPathInput) {
    const rootPath = normalizeRoot(rootPathInput);
    const startedAt = new Date().toISOString();
    const state = getDeveloperGitAutomationState(rootPath);
    if (!state.isRepository) {
...
function cleanDeveloperSnapshotStaging() {
    const stagingRoot = path.join(app.getPath("temp"), "workflow-studio-ai-snapshots");
    try {
...
ipcMain.handle("developer:getGitAutomationState", (_event, rootPath) => getDeveloperGitAutomationState(rootPath));
